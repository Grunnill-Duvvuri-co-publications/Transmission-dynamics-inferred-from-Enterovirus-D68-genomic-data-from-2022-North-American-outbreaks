...
x_max_date = ceiling_date(lubridate::date_decimal(max(ON_origin_all)), 'month')
...
plot(den_ON_origin,xlim = c(x_min_decimal, x_max_decimal), col='blue', lwd=2,
     ylab='Epidemic Origin', xaxt='n' ,xlab="",main="")
...
#axis(side=1,at=lubridate::decimal_date(date_ticks), las=2)
axis(side=1,at=lubridate::decimal_date(date_ticks), las=2, 
     labels = zoo::as.yearmon(date_ticks))
rect(xleft =lubridate::decimal_date(MD_NPI_lift_begin),
     xright =lubridate::decimal_date(MD_NPI_lift_end),
     ybottom = par("usr")[3],
     ytop = par("usr")[4], 
     border = NA, col = adjustcolor("grey", alpha=0.5))
...
axis.POSIXct(side=1,at=date_ticks,
             format = '%y-%b', las=2, labels=FALSE)
title(xlab="Year-Month", mgp=c(5,0.75,0), family="Calibri Light",cex.lab=1.2)



#### Re plots #####
x_min_date = as.POSIXct("2021-03-01", format="%Y-%m-%d") 
x_max_date = as.POSIXct("2022-11-01", format="%Y-%m-%d")
date_ticks = seq(as.POSIXct(x_min_date),as.POSIXct(x_max_date),by='months')
nf <- layout( matrix(c(1,2), ncol=1, nrow=2), heights = c(1,1.5))
...
x_min_date = floor_date(lubridate::date_decimal(min(MD_TMRCA_all,ON_TMRCA_all)),'month') 
x_max_date = ceiling_date(lubridate::date_decimal(max(MD_TMRCA_all,ON_TMRCA_all)), 'month')
date_ticks = seq(as.POSIXct(x_min_date),as.POSIXct(x_max_date),by='months')
y_min = 0
y_max = max(c(ON_Re_gridded_hpd, MD_Re_gridded_hpd))
x_min_decimal = lubridate::decimal_date(x_min_date)
